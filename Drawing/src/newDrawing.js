...
const DrawingApp = () => {
  ...
  useEffect(() => {
    socket.on("next-drawer", ({ drawer, isSelectingWord }) => {
      setCurrentDrawer(drawer);
      setIsSelectingWord(isSelectingWord);
      setSelectedWord(null);
    });

    socket.on("word-confirmed", ({ word, drawer }) => {
  ...
  const selectWord = () => {
    if (!wordInput) return;
    socket.emit("word-selected", wordInput);
    setWordInput("");
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center justify-center min-h-screen">
        {currentDrawer === user.username && !isSelectingWord ? (
        ...
        ) : currentDrawer !== user.username && !selectedWord ? (
